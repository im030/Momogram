...
for arch in "${architectures[@]}"; do
	...
	cmake \
		...
		-D OPENSSL_INCLUDE_DIR="${app_directory}/third_party/boringssl/include" \
		-D OPENSSL_CRYPTO_LIBRARY="${app_directory}/third_party/boringssl/build/${arch}/crypto/libcrypto.a" \
		-D OPENSSL_SSL_LIBRARY="${app_directory}/third_party/boringssl/build/${arch}/ssl/libssl.a" \
		-DTD_E2E_ONLY=ON \
		-B "${build_directory}" \
		-S "${source_directory}"
