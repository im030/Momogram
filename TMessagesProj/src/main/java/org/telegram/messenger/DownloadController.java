    ...
    public void onDownloadFail(MessageObject parentObject, int reason) {
        ...
        AndroidUtilities.runOnUIThread(() -> {
            ...
            if (removed) {
                getNotificationCenter().postNotificationName(NotificationCenter.onDownloadingFilesChanged);
                if (reason == 0) {
                    NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.showBulletin, Bulletin.TYPE_ERROR, LocaleController.formatString(R.string.MessageNotFound));
                } else if (reason == -1) {
