...
abstract class TelegramStringsTask : DefaultTask() {
    ...
    companion object {
        ...
        // Stable string entry IDs are assigned densely starting at this offset. Pinning
        // them near 0xFFFF (the previous behaviour) forced aapt2 to size the string
        // type's entry array up to 0xFFFF for every configuration, which bloated
        // resources.arsc by ~19 MB. The low slots stay free for AppName/AppNameBeta
        // and other string resources that are not part of the stable-id list.
        private const val STRING_RESOURCE_ID_OFFSET = 0x400
        private const val STRING_RESOURCE_ID_BASE = 0x7F0F0000 + STRING_RESOURCE_ID_OFFSET
    }
    ...
    fun generate() {
        ...
        // Remove the previous resource-shrinker discard rules. Keeping them
        // would make aapt2 strip every @string even though code still calls
        // Resources.getString(R.string.*) directly.
        stringsDir.resolve("raw/strings_discard.xml").delete()
    ...
    private fun generateStableIds(
        ...
        if (stableStrings.size + STRING_RESOURCE_ID_OFFSET > 0x10000) {
            error(
